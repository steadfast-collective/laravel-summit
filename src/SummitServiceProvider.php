    ...
    protected function registerPublishables() : void
    {
        if ($this->app->runningInConsole()) {
            ...
            if (! class_exists('CreateCourseBlocksTable')) {
                $this->publishes([
                    __DIR__.'/../database/migrations/create_course_blocks_table.php.stub' => database_path('migrations/'.date('Y_m_d_His', time() + 60).'_create_course_blocks_table.php'),
                ], 'summit-migrations');
            }

            if (! class_exists('CreateCourseBlockUserTable')) {
                $this->publishes([
                    __DIR__.'/../database/migrations/create_course_block_user_table.php.stub' => database_path('migrations/'.date('Y_m_d_His', time() + 120).'_create_course_block_user_table.php'),
                ], 'summit-migrations');
            }
        }
    }
